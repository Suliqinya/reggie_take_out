...
import com.gk.reggie.utils.SMSUtils;
import com.gk.reggie.utils.ValidateCodeUtils;
...
public class UserController {
    ...
    public R<String> sendMsg(@RequestBody User user, HttpSession session) {
        ...
        if (StringUtils.isNotEmpty(phone)) {
            ...
            log.info("生成的验证码： code = {}", code);
            //调用阿里云提供的短信服务API完成发送短信
            SMSUtils.sendMessage("瑞吉外卖","SMS_461160103",phone,code);

            //需要将生成的验证码保存到Session
//            session.setAttribute(phone, code);

            //将生成的验证码缓存到Rdis中，并且设置有效期为5分钟
